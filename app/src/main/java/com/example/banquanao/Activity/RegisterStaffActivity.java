...
public class RegisterStaffActivity extends AppCompatActivity {
    ...
    EditText edPass, edRePass, edName, edPhone, edMail,edBirthday;
    boolean Visible;
    ...
    private void addControll() {
        edName = findViewById(R.id.editTextName_ReStaff);
        edPhone = findViewById(R.id.editTextPhone_ReStaff);
        edMail = findViewById(R.id.editTextEmail_ReStaff);
        edPass = findViewById(R.id.editTextNewPass_ReStaff);
        edRePass = findViewById(R.id.editTextRePass_ReStaff);
        edBirthday = findViewById(R.id.editTextBirthday_ReStaff);
        btLogin= findViewById(R.id.buttonLogin_ReStaff);
        clickRegister();
    }
    private void clickRegister() {
        btRegister = findViewById(R.id.buttonRegister_ReStaff);
        btRegister.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                checkVarRegisterStaff();
            }
        });
    }
    private void checkVarRegisterStaff(){
        String name, email, pass, repass,birthday;
        int phone = 0;
        ...
        birthday=edBirthday.getText().toString().trim();
        if(name.equals("") && phone==0 && email.equals("") && pass.equals("") && repass.equals("")){
        ...
        if(birthday.equals("")){
            Toast.makeText(getApplicationContext(),"Vui lòng nhập sinh nhật",Toast.LENGTH_SHORT).show();
            return;
        }
        insertStaff(name, repass, phone, email,birthday);
    }

    private void insertStaff(String name , String pass, int phone, String email,String birthday){
        ContentValues values = new ContentValues();
        values.put("id_Staff", getMaxIdStaff());
        values.put("name", name);
        values.put("passWork", pass);
        values.put("email", email);
        values.put("number_phone", phone);
        values.put("idPowerStaff", 1);
        values.put("birthday",birthday);
        values.put("idManager",0);
        values.put("image","ss");
        long kq = PageMainLoginActivity.database.insert("Staff",null, values);
        if(kq > 0){
            Toast.makeText(getApplicationContext(),"Cập nhật thành công",Toast.LENGTH_SHORT).show();
            Intent i = new Intent(RegisterStaffActivity.this, LoginActivity.class);
            startActivity(i);
    ...
    public static boolean isPhoneExists(int phoneEnter){
        String query = "select number_phone from Staff";
        Cursor cursor = PageMainLoginActivity.database.rawQuery(query,null);
        while (cursor.moveToNext()){
    ...
    private void eventBack() {
        btBack = findViewById(R.id.buttonBack_ReStaff);
        btBack.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent i = new Intent(RegisterStaffActivity.this, PageMainLoginActivity.class);
                startActivity(i);
            }
        });
    }
